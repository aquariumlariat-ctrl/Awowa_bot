...
//
// Si Imgur falla, no lo bombardeamos. Esperamos IMGUR_RETRY_MS antes de
// volver a intentar. Si el servicio se recupera solo, el bot también.
// ─────────────────────────────────────────────────────────────────────────────
const IMGUR_RETRY_MS = 5 * 60 * 1000; // 5 minutos antes de reintentar

let avatarFijoBuffer  = null;
let avatarFijoFailedUntil = 0; // 0 = nunca ha fallado, puede intentar

// Para los iconos de rango usamos un objeto indexado por número de rank
const rankBuffers    = { 1: null, 2: null, 3: null };
const rankFailedUntil = { 1: 0,    2: 0,    3: 0 };
const RANK_URLS = {
    1: 'https://i.imgur.com/cOmyYfQ.png',
    2: 'https://i.imgur.com/AsvLb0x.png',
...
async function loadAvatarFijo() {
    // Circuit breaker: si falló recientemente, no reintentamos todavía
    if (Date.now() < avatarFijoFailedUntil) {
        throw new Error(`Circuit Breaker activo para avatar. Reintento en ${Math.ceil((avatarFijoFailedUntil - Date.now()) / 1000)}s`);
    }

    if (!avatarFijoBuffer) {
        try {
            avatarFijoBuffer = await fetchBuffer('https://i.imgur.com/48MynuQ.png');
        } catch (error) {
...
async function loadRankIcon(rank) {
    if (!RANK_URLS[rank]) return null;

    // Circuit breaker por rank individual
    if (Date.now() < rankFailedUntil[rank]) {
        throw new Error(`Circuit Breaker activo para rank icon ${rank}.`);
    }

    if (!rankBuffers[rank]) {
        try {
            rankBuffers[rank] = await fetchBuffer(RANK_URLS[rank]);
        } catch (error) {
    ...
    return await loadImage(rankBuffers[rank]);
}
...
async function generarCanvasNivel(socialData, discordNick, discordUsername, userId = null) {
    ...
    try {
        ...
        // Imgur caído o circuit breaker activo: dibujamos recuadro negro elegante
    ...
    // 2. RANGO (solo si tenemos posición real)
    if (rankPos !== "-") {
        ...
        // 3. ICONOS TOP 3 (con circuit breaker propio, fallo silencioso)
        if (typeof rankPos === 'number' && rankPos <= 3) {
            ...
            try {
                ...
                // Circuit breaker activo o Imgur caído: simplemente no dibujamos el ícono
            }
        }
    }
